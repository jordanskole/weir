...
// Two distinct edges so an allOf node has more than one declared input —
// the shape needed to prove a replayed allOf invocation's causationIds
// pass straight through, the same as a single-input node's do.
const A: AnyEdgeDef = {
  name: "A",
  label: "A",
  description: "Edge A",
...
// recorded causation rather than silently reverting to `invokeWithInput`'s
// default of `[]`.
...
describe("replayInvocation", () => {
  ...
  it("replays an allOf node under its recorded causationIds, passed straight through both times", async () => {
    ...
    // Specific, non-empty ids a caller (the runtime, resolving readiness
    // against a real Log) would have supplied — membrane no longer derives
    // these itself, for `allOf` any more than for `single` (Task 4).
    const idA = "inst-a";
    const idB = "inst-b";
    const invocation = await membrane(nodeDef, { A: { value: "a" }, B: { value: "b" } }, {
      correlationId: "c-allof-causation",
      causationIds: [idA, idB],
    });
    ...
    // The recorded causationIds ([idA, idB]) come back unchanged: neither
    // the original invocation above nor `replayInvocation` resolves
    // causation internally any more, so there is nothing left to disagree
    // with the recorded value — both just pass through what they're given.
