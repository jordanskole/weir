...
**Left open:** the version-pin field's exact name/shape on the envelope; whether `.node.yaml` and its implementation directory need a structural package boundary between them (the way my earlier project really does separate schemas from runtime) or just live side by side; the `oneOf`/`allOf`/`many` YAML shape inside `output:` beyond the `single`-sugar case sketched in passing; JSON Schema generation itself, still deferred.
