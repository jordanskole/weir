...
**Left open:** the version-pin field's exact name/shape on the envelope; the `oneOf`/`allOf`/`many` YAML shape inside `output:` beyond the `single`-sugar case sketched in passing; JSON Schema generation itself, still deferred.
