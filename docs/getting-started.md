...
3. **An authoring format** — `.edge`/`.node`/`.topology` as real YAML files, plus the versioned-implementation-directory mechanism for `Fn` bodies. Design settled in [design.md](design.md#10-authoring-format) (§10) and [design-history.md](design-history.md#composition-parser-deferred-authoring-format-designed-instead); not yet built. Replaces the composition-parser step this build order originally had here — see that design-history entry for why.
4. **An elaborator** that emits the netlist matching what was hand-written in step zero, reads the `.edge`/`.node`/`.topology` files from step 3, and scaffolds/wires each node's implementation-directory seam. This is also where generics get resolved — see "Generics: elaboration monomorphizes" in [design-history.md](design-history.md) — so treat the elaborator as load-bearing, not a thin convenience layer.
5. **A runtime** that walks the netlist, calls each node's `Fn` (against the implementation version an invocation was pinned to on replay), and appends the resulting edge instances to the log.
...
- **JSON Schema generation for `.edge`/`.node`/`.topology`** — the editor tooling (validation, completion, hover) step 3's design calls for, mechanically derivable from the same types once they exist. Not needed to hand-author or elaborate the files.
