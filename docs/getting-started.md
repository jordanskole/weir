...
1. **Edge definitions — done.** Stole the `defineField` / `defineDataset` pattern from [bankql](../../bankql) wholesale — verified against `packages/schema/src/define.ts` and `types.ts`. Implemented at [`src/types.ts`](../src/types.ts), [`src/define.ts`](../src/define.ts), [`src/hash.ts`](../src/hash.ts) (renamed `Dataset`→`Edge`; `hashEdge()` reproduces bankql's structural-fingerprint fields exactly — see [design-history.md](design-history.md#prior-art-bankql-already-proves-the-edge-half)). 20 tests in `src/*.test.ts`, all passing; `npm test` / `npm run typecheck` both green.
2. **Node declarations.** Name, in-edge, out-edge, tests, `Fn` reference. Next up — should follow the shape the step-zero netlist's `nodes` block already sketches, once that's confirmed.
