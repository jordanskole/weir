...
**Architecture:** Two repos, not a monorepo. `weir` (this repo) stays the language/compiler — no hosting, no UI, no auth, publishable as clean OSS. A new sibling repo, `kleisli`, is the hosted platform: the web UI, auth, and (in later phases) the Inngest-orchestrated, Lambda/Deno-Sandbox-executed topology execution runtime. `kleisli` depends on `weir` as a package once weir has one to depend on; nothing does yet (weir has no root `package.json` — only `spikes/ts-prototype` does), so Phase 0 below scaffolds `kleisli` standalone and defers the dependency wiring to whichever phase actually needs to call weir's compiler.

**Tech Stack:** Next.js (web UI) + Clerk (auth) + Vercel (UI hosting) for the platform frontend; Inngest (durable orchestration) + AWS Lambda or Deno Sandbox (isolated custom-code execution) for the topology execution runtime, per the 2026-09-10 hosting discussion — not built in this plan, listed here so Phase 0's stack choices don't contradict them later.
