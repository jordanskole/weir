...
- the declared `OutputSpec`, checked structurally: `single` → `assertPayload` against the output edge directly; `oneOf` → the tagged branch's `.edge` must be one of the declared edges, its `.payload` asserted against that edge; `allOf` → every declared edge present and tagged, each asserted; `many` → the result is a bare `Record<string, PayloadOf<E>>` (a keyed collection, *not* a single edge payload — `OutputResult`'s own `many` case, `types.ts`), so it needs its own per-entry check: each entry asserted against the output edge, then its own declared `index` field checked against the key it's stored under — the same per-entry logic `assertPayload`'s existing `many`-*field* branch already runs for a `many` field nested inside an edge, adapted here for a `many` *output* standing alone with no enclosing field to nest under. (No existing function does this dispatch-and-assert — `runtime.ts`'s `logOutput` blindly casts without validating. This is genuinely new, reusing `assertPayload` per branch rather than duplicating its logic.)
