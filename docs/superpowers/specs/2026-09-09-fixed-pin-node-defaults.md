...
- **Caller-facing schema**: a field named in `pin` is *absent*, not merely optional, from `given` in an example (and, eventually, from a real invocation payload) — present-with-any-value is a validation error, mirroring how a `literal` edge field is never caller-suppliable.
- **`Fn`'s type is unchanged.** `InputPayload<In>` still reflects the full input edge — `pin` doesn't narrow what `Fn` receives or is typed to return; `Fn` is responsible for actually producing the pinned value(s) in whatever it returns. No runtime injection/merge is built here — declarative and example-checked only, exactly matching how `expected`/`literal` work today (neither is mechanically enforced by the runtime; both are checked by examples passing).
- **Examples**: `given` omits every field named in `pin`. `expect`'s value for a pinned field, when the output edge has a field of that name, must structurally equal the declared `pin` value — checkable at schema/parse time, independent of running `Fn`.
